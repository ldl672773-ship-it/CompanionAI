...
type ChatDrawerSearchItemProps = {
    ...
    onLoad: (id: number, setOffset?: { type: 'index' | 'messageId'; value: number }) => void
}

const ChatDrawerSearchItem: React.FC<ChatDrawerSearchItemProps> = ({ item, onLoad, query }) => {
    const { color, spacing, fontSize } = Theme.useTheme()

    const segmented = segmentText(item.content, query)

    return (
        <TouchableOpacity
            onPress={() => onLoad(item.chatId, { type: 'messageId', value: item.messageId })}
            style={{
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                ...
                <Text style={{ color: color.text._600, fontSize: fontSize.s }}>
                    {getFriendlyTimeStamp(item.createdAt.getTime())}
                </Text>
            </View>
            <Text>
