...
const initLlama = async (options: any): Promise<LlamaContext> => null
const Llama = { load: async () => null }
...
export namespace Tokenizer {
    ...
    export const getTokenizer = () => {
        ...
        if (appMode === 'local') {
            const localTokenizer = Llama.useLlamaModelStore.getState()?.tokenLength
            if (!localTokenizer) {
                Logger.warn('Local tokenizer unavailable, using default tokenizer')
    ...
    export const useTokenizer = () => {
        const defaultTokenizer = useTokenizerState((state: TokenizerState) => state.getTokenCount)
        const llamaTokenizer = Llama.useLlamaModelStore((state: any) => state.tokenLength)
        const appMode = useAppModeStore((state: any) => state.appMode)
