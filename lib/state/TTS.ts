...
type SpeechVoice = { identifier: string; name: string; language: string }
...
useInference.subscribe(({ nowGenerating }) => {
    ...
    if (!nowGenerating) {
        ...
        if (!message) return
        useTTSStore.getState().handleEndGeneration(length - 1, message.content)
    } else {
